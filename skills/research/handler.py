"""Research Skill — autonomously researches web design trends, animation
techniques, and tools.

Ported from openclaw.agents.research.ResearchAgent.
"""
...
RESEARCH_SYSTEM_PROMPT = """You are the Research Agent of Clarmi Design Studio, a digital design agency.
...
class ResearchSkill(BaseSkill):
    name = "research"
    description = "Autonomously researches web design trends, animation techniques, and tools"
    tier = "light"
    system_prompt = RESEARCH_SYSTEM_PROMPT
    tools = [SCRAPE_TOOL, STORE_KNOWLEDGE_TOOL]
    max_turns = 5  # Limit research skill turns to conserve Firecrawl credits
    timeout = 600
    MAX_SCRAPES_PER_TASK = 3  # Hard cap on Firecrawl calls per research task

    def __init__(self, claude_client):
        super().__init__(claude_client)
        self._scrape_count = 0  # Track scrapes per task

    async def execute(self, context: SkillContext) -> SkillResult:
        """Override to reset scrape counter per task."""

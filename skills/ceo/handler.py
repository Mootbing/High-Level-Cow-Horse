"""CEO Skill — top-level orchestrator that receives owner messages via WhatsApp or Dashboard."""
...
class CEOSkill(BaseSkill):
    name = "ceo"
    description = "Top-level orchestrator that receives owner messages via WhatsApp or Dashboard"
    tier = "light"
    ...
    async def execute(self, context: SkillContext) -> SkillResult:
        """Process incoming messages from WhatsApp, Dashboard, or skill results."""
        ...
        if msg_type == "dashboard_message":
            ...
            # Another skill completed a task and is reporting back
        ...
        return SkillResult(success=True, result=response, data={"status": "completed", "response": response})

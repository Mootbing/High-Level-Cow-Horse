"""Project Manager Skill — manages sequential pipeline: inbound, design, build, QA, outbound."""
...
class ProjectManagerSkill(BaseSkill):
    ...
    description = "Manages sequential pipeline - inbound, design, build, QA, outbound"
    tier = "light"
    ...
    async def execute(self, context: SkillContext) -> SkillResult:
        """Capture project_id from the incoming message and init designer assets list."""
        ...
        # Use the default execute flow (extract prompt, call run())
        if not self._current_project_id:
            self._current_project_id = context.project_id
        payload = context.payload
        prompt = payload.get("prompt", json.dumps(payload))
        context_msgs = payload.get("context", [])
        ...
        response = await self.run(prompt, context_msgs)
        return SkillResult(success=True, result=response, data={"status": "completed", "result": response})
